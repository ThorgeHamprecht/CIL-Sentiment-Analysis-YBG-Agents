...
run_variant() {
    ...
    local artifact_dir="$SCRATCH/artifacts/28_contrastive_shared_head_${supcon_variant}_${weight_tag}_${warmup_tag}"
    local submission_prefix="28_shared_head_${supcon_variant}_${weight_tag}_${warmup_tag}"

    mkdir -p "$artifact_dir"

    echo ""
    echo "=== 28 shared-head: variant=$supcon_variant weights=$weight_tag warmup=$warmup_epochs ==="
    echo "Running 28 shared-head variant=$supcon_variant weights=$weight_tag warmup=$warmup_epochs w1=$w1_weight supcon=$supcon_weight"
