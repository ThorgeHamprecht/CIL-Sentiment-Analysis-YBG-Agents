"""Train mDeBERTa-v3-base with shared contrastive representation and W1 head."""
import argparse
import math
import os
import time
...
from model import EMA, SupervisedContrastiveLoss, SharedHeadContrastiveMDeBERTa, emd_loss, median_decode
...
def build_optimizer(model, shared_head_lr, rating_head_lr, encoder_top_lr, layer_decay, weight_decay):
    """LLRD: shared head + rating head > top encoder layer > ... > embeddings."""
    no_decay = {"bias", "LayerNorm.weight", "LayerNorm.bias"}
    groups = []

    _add_param_groups(groups, list(model.shared_head.named_parameters()), shared_head_lr, weight_decay, no_decay)
...
def main(args):
    ...
    optimizer = build_optimizer(
        model,
        shared_head_lr=args.shared_head_lr,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--shared_head_lr",   type=float, default=1e-4)
    ...
    parser.add_argument("--save_epoch_checkpoints", type=int, nargs="*", default=[4])
