"""mDeBERTa-v3-base with shared contrastive representation head and W1 classifier."""
import torch
...
class SharedHeadContrastiveMDeBERTa(nn.Module):
    """Shared representation head feeding both SupCon and W1 losses."""
    ...
    def __init__(
        ...
        self.rating_head = nn.Linear(representation_dim, num_classes)

    def forward(self, input_ids, attention_mask):
        out = self.encoder(input_ids=input_ids, attention_mask=attention_mask)
        pooled = mean_pool(out.last_hidden_state, attention_mask)
        representation = self.shared_head(pooled)
        embeddings = F.normalize(representation, p=2, dim=-1)
        logits = self.rating_head(representation)
        return {
            "logits": logits,
