"""Fine-tune mDeBERTa-v3-base with CORAL ordinal loss."""
import argparse
import os
import time
...
from model import coral_loss, mDeBERTaCORAL
...
def main(args):
    ...
    model = mDeBERTaCORAL(model_name=MODEL_NAME, dropout=args.dropout).to(device)
    print(f"Parameters: {sum(p.numel() for p in model.parameters()):,}")

    optimizer = torch.optim.AdamW(
        [
            {"params": model.encoder.parameters(), "lr": args.encoder_lr},
            {"params": list(model.fc.parameters()) + [model.bias], "lr": args.head_lr},
        ],
        weight_decay=0.01,
    )
