...
Each seed gets its own independent 90/10 stratified train/val split, so both
the training data order and the validation set differ across seeds — mimicking
k-fold diversity with fewer folds.
"""
import argparse
import time
...
def run_seed(seed, texts, labels, tokenizer, args, out_dir, device):
    # Each seed gets its own independent 90/10 stratified split
    ...
    print(f"  [seed={seed}] train={len(train_texts):,}  val={len(val_texts):,}")
...
def main(args):
    ...
    for seed in seeds:
        print(f"\n=== Seed {seed} (independent 90/10 split) ===")
        run_seed(seed, texts, labels, tokenizer, args, out_dir, device)
