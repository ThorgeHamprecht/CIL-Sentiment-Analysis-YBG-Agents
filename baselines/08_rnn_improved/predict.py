"""Run the trained AttentionBiLSTM on test.csv and write a Kaggle submission CSV."""
import argparse
...
def predict(model, loader, device) -> np.ndarray:
    ...
    for batch in loader:
        ...
        all_preds.append(model(x, lengths).argmax(dim=1).cpu().numpy())
...
def main(args):
    ...
    out_path = output_dir / "08_rnn_improved_submission.csv"
