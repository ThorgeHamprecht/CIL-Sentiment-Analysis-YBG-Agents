#!/bin/bash
#SBATCH --job-name=mdeberta_emd
#SBATCH --output=/work/scratch/%u/cil/logs/mdeberta_emd-%j.out
#SBATCH --error=/work/scratch/%u/cil/logs/mdeberta_emd-%j.err
...
mkdir -p \
    "$SCRATCH/artifacts/19_mdeberta_emd" \
    "$SCRATCH/submissions" \
    "$SCRATCH/.cache/torch" \
...
python train.py \
    ...
    --head_lr     1e-4 \
    --dropout     0.1 \
    --artifact_dir "$SCRATCH/artifacts/19_mdeberta_emd" \
    --data_dir     "$SCRATCH/data"

# ── Predict + submission ──────────────────────────────────────────────────────
python predict.py \
    --checkpoint  "$SCRATCH/artifacts/19_mdeberta_emd/best_model.pt" \
    --artifact_dir "$SCRATCH/artifacts/19_mdeberta_emd" \
    --data_dir    "$SCRATCH/data" \
