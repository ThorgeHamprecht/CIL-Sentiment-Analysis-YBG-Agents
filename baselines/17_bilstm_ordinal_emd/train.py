"""Train the two-stream BiLSTM with EMD² loss and median decode."""
import argparse
import math
import time
...
from model import TwoStreamBiLSTM, emd_loss, median_decode

ROOT = Path(__file__).resolve().parents[2]
_DEFAULT_DATA_DIR = ROOT / "data"
_DEFAULT_ARTIFACT_DIR = Path(__file__).parent / "artifacts"

AUX_WEIGHT = 0.3  # auxiliary title-head loss weight
...
def evaluate(model, loader, device):
    ...
    return kaggle_score(preds, labels)
...
def main(args):
    ...
    if vocab_path.exists():
        ...
        all_texts = [t + " " + b for t, b in zip(titles, bodies)]
    ...
    tr_titles, tr_bodies, tr_labels = subset(train_idx)
    va_titles, va_bodies, va_labels = subset(val_idx)

    train_ds = TwoStreamDataset(tr_titles, tr_bodies, vocab, args.max_len_title, args.max_len_body, tr_labels)
    val_ds   = TwoStreamDataset(va_titles, va_bodies, vocab, args.max_len_title, args.max_len_body, va_labels)
