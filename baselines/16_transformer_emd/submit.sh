#!/bin/bash
#SBATCH --job-name=transformer_emd
#SBATCH --output=/work/scratch/%u/cil/logs/transformer_emd-%j.out
#SBATCH --error=/work/scratch/%u/cil/logs/transformer_emd-%j.err
...
mkdir -p \
    "$SCRATCH/artifacts/16_transformer_emd" \
    "$SCRATCH/submissions" \
    "$SCRATCH/.cache/torch" \
    "$SCRATCH/logs"
...
# ── Train ─────────────────────────────────────────────────────────────────────
BASELINE_DIR="/home/$USER/CIL-Sentiment-Analysis-YBG-Agents/baselines/16_transformer_emd"
cd "$BASELINE_DIR"
...
python train.py \
    ...
    --max_len         256 \
    --artifact_dir "$SCRATCH/artifacts/16_transformer_emd" \
    --data_dir     "$SCRATCH/data"

# ── Predict + submission ──────────────────────────────────────────────────────
python predict.py \
    --checkpoint  "$SCRATCH/artifacts/16_transformer_emd/best_model.pt" \
    --vocab       "$SCRATCH/artifacts/16_transformer_emd/vocab.json" \
    --data_dir    "$SCRATCH/data" \
