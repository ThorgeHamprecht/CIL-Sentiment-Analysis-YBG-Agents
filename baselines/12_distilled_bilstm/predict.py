"""Generate submission CSV from the distilled BiLSTM checkpoint."""
import argparse
...
from dataset import DistillDataset, load_vocab, read_csv
from model import TwoStreamBiLSTM, ev_decode
...
def main(args):
    ...
    # Dummy soft labels (zeros) — not used during inference
    dummy_soft = np.zeros((len(titles), 5), dtype=np.float32)
    ds = DistillDataset(titles, bodies, vocab, dummy_soft,
                        saved_args["max_len_title"], saved_args["max_len_body"])
    loader = DataLoader(ds, batch_size=512, shuffle=False, num_workers=2, pin_memory=True)

    all_preds = []
    with torch.no_grad():
        for x_t, l_t, x_b, l_b, _, _, _ in loader:
    ...
    out = sub_dir / "submission_12_distilled_bilstm.csv"
